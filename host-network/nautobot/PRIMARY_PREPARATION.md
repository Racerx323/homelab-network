...
archival is published and verified; do not rerun the consumed bundle. The preparation below
...
`nautobot-standby-route-accepted`. The archive and annotated tag are published and verified. The [primary operation](PRIMARY_OPERATION.md) is implemented and locally tested;
its exact execution bundle requires separate approval. The authorized
[read-only preflight](PRIMARY_PREFLIGHT.md) passed on both nodes; primary recovery
availability is confirmed. The consumed standby bundle must not be reused or edited.
...
1. Read-only preflight on both nodes: capture boot IDs, active profile UUID/path,
   ...
   Primary console/physical recovery was confirmed during preflight; preserve
   that availability during execution. Resolve controller tools and certificate trust.
...
These phases are implemented in the existing Ansible/helper path, using explicit
node identity and expected role per phase. Add focused tests for failed handoff (no route
...
The bounded read-only preflight on both nodes is complete. Implementation and local tests are complete; next is approval of the frozen
primary operation bundle. The
preflight made no route/service/HA changes or Nautobot storage queries. Live handoff and primary mutation need
