...
def transaction(action, p, host, uuid, call=run, save=write_state, root=BASE, snapshot=profile_snapshot, restore=restore_profile, locate=find_profile, primary=False):
    commands = route_commands(p, host, uuid)
    expected = ('pihole0', '178f6c60-c62a-3c6c-9fae-04ebf2c4fdc3') if primary else ('pihole00', '3078e1cc-2e08-3745-b5e4-a60426628c39')
    if (host, uuid) != expected:
    ...
    if action in {'accept', 'expire'}:
        ...
        if action == 'accept':
            ...
            expected_addresses = set(existing['addresses_before'])
            if primary:
                expected_addresses |= {'10.1.0.55','10.1.0.56','fd36:5aa8:6971:1::55','fd36:5aa8:6971:1::56'}
            if {x['local'] for row in now for x in row.get('addr_info', [])} != expected_addresses:
