...
def verify(bundle, approved):
    ...
    if manifest['kind'] not in {'standby_route_retry','primary_route'} or manifest['execution_ready'] is not True:
...
def main():
    ...
    primary = manifest['kind'] == 'primary_route'
    address = '10.1.0.53' if primary else '10.1.0.54'
    host = 'pihole0' if primary else 'pihole00'
    uuid = '178f6c60-c62a-3c6c-9fae-04ebf2c4fdc3' if primary else '3078e1cc-2e08-3745-b5e4-a60426628c39'
    playbook = 'ansible/primary-route.yaml' if primary else 'ansible/route.yaml'
    cmd=['ansible-playbook','-i',address+',','-u','pi',playbook,'--extra-vars',
         json.dumps({'candidate_execution_enabled':True,'route_host':host,
                     'route_uuid':uuid,'route_qualification_verified':True,
                     'ha_standby_verified':True,'controller_evidence':str(root)})]
    if primary:
        import primary_health
        import health_watch
        monitor = Monitor(root,probe=lambda:health_watch.sample(node_probe=lambda a:health_watch.node(a,extended=True)),evaluator=primary_health.from_root)
        health_gate = primary_health.gate
    else:
    ...
    error = None
    try:
        health_gate(root, 'healthy', timeout=15)
        with open(root/'stdout.log','w') as stdout, open(root/'stderr.log','w') as stderr:
            result = subprocess.run(cmd,cwd=root,env=env,stdout=stdout,stderr=stderr,timeout=900 if primary else 600)
        result_rc = result.returncode
        report = subprocess.run(['ssh', '-o', 'BatchMode=yes', '-o', 'StrictHostKeyChecking=yes',
                                 '-o', 'ConnectTimeout=10', 'pi@'+address,
                                 ('cd / && sudo -n /usr/bin/python3 -I /usr/local/lib/nautobot-network/primary_route.py report' if primary else
        ...
        if accepted:
            health_gate(root, 'stable', timeout=10)
